    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if ((HORIZONTAL & mOrientation.value) == HORIZONTAL) {
                ...
                drawLine(horizontalDrawable, c, left1, child.getTop() - params.topMargin, left1 + mWidth, child.getBottom() + params.bottomMargin);
                drawLine(horizontalDrawable, c, right1, child.getTop() - params.topMargin, right1 + mWidth, child.getBottom() + params.bottomMargin);
            }
            if ((VERTICAL & mOrientation.value) == VERTICAL) {
                ...
                drawLine(verticalDrawable, c, child.getLeft() - params.leftMargin - mWidth, top1, child.getRight() + params.rightMargin + mWidth, top1 + mHeight);
                drawLine(verticalDrawable, c, child.getLeft() - params.leftMargin - mWidth, bottom1, child.getRight() + params.rightMargin + mWidth, bottom1 + mHeight);
            }
        }
    }

    private void drawLine(Drawable drawable, Canvas canvas, int left, int top, int right, int bottom) {
        if (paint == null) {
